...
ids = [[line[:-1].split('\t')[0], line[:-1].split('\t')[3],line[:-1].split('\t')[4]] for line in open(PATH_input+'kuleshov_info.txt').readlines()]

target_mutation = []
target_ids = []

for mutation in database_genes:
    if mutation[0] == 'snp' and mutation[1] == 'rpsL' and mutation[2] == '43' and mutation[4] == 'K' and mutation[5] == 'R':
        target_mutation.append(mutation)

for element in mutation_info:
    for mutation in element[1]:
        if mutation in target_mutation:

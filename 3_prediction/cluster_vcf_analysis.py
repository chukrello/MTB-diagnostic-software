...
def look_vcf_file(filename, name, upstream_snp, file):
    ...
    file.write(name+'\n')
    # open vcf file
    raw_data_1 = [[int(line.split('\t')[1]), line.split('\t')[3], line.split('\t')[4]] for line in open(filename).readlines() if line[0] != '#']
    raw_data_2 = [[int(line.split('\t')[1]), line.split('\t')[3], line.split('\t')[4]] for line in open(filename[:-4]).readlines() if line[0] != '#' and float(line.split('\t')[5]) >= 600.0]
    data = []
    # for el in raw_data_1:
    #     if el not in data:
    #         data.append(el)
                    ...
                    for walker_mut in database_genes:
                        if walker_mut[0] == 'snp':
                            if walker_mut[1] == cur_gene and walker_mut[2][0] != '-' and int(walker_mut[2]) == protein_pos:
                                ...
                                if Seq(old_triplet).translate() != walker_mut[3]:
                                    print(walker_mut)
                                    print(mutation)
                            ...
                            if walker_mut[0] == 'snp':
                                if walker_mut[1] == cur_gene and walker_mut[2][0] != '-' and int(walker_mut[2]) == protein_pos:
                                    ...
                                    if (Seq(old_triplet).translate() == walker_mut[3]) and (Seq(new_triplet).translate() == walker_mut[4]):
                                        ...
                                        if ' '.join(walker_mut) == 'snp rpsA 260 V I PZA Resistant':
                                            print(name)
                                            print(old_triplet)
                                            print(new_triplet)
                                            print(mutation)
...
file = open('statistics_snp.txt', 'w')

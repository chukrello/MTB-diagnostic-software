# IMPORTANT SCRIPT
# Uses Kuleshov and Vphaser data, uses Walker or new dictionary
# In 20.02
...
# determing type of dictionary and samples
if sys.argv[1] == 'Walker':
	isWALKERDICT = True

if sys.argv[2] == 'Walker':
	isWALKERIDS = True
...
def make_mutations_database():
	mutations = []
	if isWALKERDICT == False:
...
database_mutations = make_mutations_database()
...
def look_all_files(filename_1, filename_2, name, upstream_snp, file):
	
	print('Looking at ' + name)
	file.write(name+'\n')

	look_vcf_file(filename_1, name, upstream_snp, file)
	look_vphaser_file(filename_2, name, upstream_snp, file)

current_list = []

if isWALKERIDS == True:
	current_list = walker_names
	if isWALKERDICT == True:
		file = open('results_walker_walker_dict.txt', 'w')
	else:
		file = open('results_walker_new_dict.txt', 'w')
else:
	current_list = nonwalker_names
	if isWALKERDICT == True:
		file = open('results_nonwalker_walker_dict.txt', 'w')
	else:
		file = open('results_nonwalker_new_dict.txt', 'w')


for name in current_list:
	look_all_files('/export/data/kkuleshov/myc/sra/' + name + '/' + name + '_h37rv.vcf.unf', '/export/data/kchukreev/9_minor_variants/vcfs/' + name + '_vphaser.vcf', name, upstream_snps, file)
